const Service = require('../Services/Database/Voyage');
    ...
    get: (req, res, next) => {
        ...
        .then((result) => {
            if (result) {
                ...
                let response = {status: 400, content: {success:false, message:"ERR_VOYAGE_DOESNT_EXISTS"}};
                res.status(response.status).json(response.content);
            }
        }).catch((err) => {
    ...
    insert : (req, res, next) => {
        ...
        .then((result) => {
            if(result) {
                ...
                let response = {status: 400, content: {success:false, message:"ERR_VOYAGE_INSERT"}};
                return res.status(response.status).json(response.content)
            }
        }).catch((err) => {
    ...
    update : (req, res, next) => {
        ...
        .then((result) => {
            if(result) {
                ...
                let response = {status: 400, content: {success:false, message:"ERR_VOYAGE_UPDATE"}};
                return res.status(response.status).json(response.content)
            }
        }).catch((err) => {
            let response = {status: 400, content: {success:false, message:err.message}};
